...
xDPC = PluginClass:new({
    name = "xDPC",
})

local xDPCPrefix = "^xDPC,reset;"
...
function xDPC:addCustomCommandPreview(availableCommands, substr)
...
local function setTextHint(mode, override)
    ...
    if override or mode ~= "xDPC::Prox" then
...
function xDPC:registerMessageHandlers(shared) --look at this function in irden chat's editchat thing
    ...
    starcustomchat.utils.setMessageHandler("/dynamicsccrp", function(_, _, data)
        if string.lower(data) == "on" then
            root.setConfiguration("xDPC::handleSccrpProx", true)
            return "^green;ENABLED^reset; handling SCCRP Proximity messages as xDPC chat"
        elseif string.lower(data) == "off" then
            root.setConfiguration("xDPC::handleSccrpProx", false)
            return "^red;DISABLED^reset; handling SCCRP Proximity messages as xDPC chat"
        else
            local enabled = root.getConfiguration("xDPC::handleSccrpProx") or false
    ...
    starcustomchat.utils.setMessageHandler("/proxooc", function(_, _, data)
        if string.lower(data) == "on" then
            root.setConfiguration("xDPC::proximityOoc", true)
            return "^green;ENABLED^reset; handling (( )) as range-limited OOC chat"
        elseif string.lower(data) == "off" then
            root.setConfiguration("xDPC::proximityOoc", false)
            return "^red;DISABLED^reset; handling (( )) as range-limited OOC chat"
        else
            local enabled = root.getConfiguration("xDPC::proximityOoc") or false
    ...
    starcustomchat.utils.setMessageHandler("/proxlocal", function(_, _, data)
        if string.lower(data) == "on" then
            root.setConfiguration("xDPC::localChatIsProx", true)
            return "^green;ENABLED^reset; handling local chat as proximity chat"
        elseif string.lower(data) == "off" then
            root.setConfiguration("xDPC::localChatIsProx", false)
            return "^red;DISABLED^reset; handling local chat as proximity chat"
        else
            local enabled = root.getConfiguration("xDPC::localChatIsProx") or false
    ...
    starcustomchat.utils.setMessageHandler("/sendlocal", function(_, _, data)
        if string.lower(data) == "on" then
            root.setConfiguration("xDPC::sendProxChatInLocal", true)
            return "^green;ENABLED^reset; sending proximity chat as local chat"
        elseif string.lower(data) == "off" then
            root.setConfiguration("xDPC::sendProxChatInLocal", false)
            return "^red;DISABLED^reset; sending proximity chat as local chat"
        else
            local enabled = root.getConfiguration("xDPC::sendProxChatInLocal") or false
    ...
    starcustomchat.utils.setMessageHandler("/chid", function(_, _, data)
        local status, resultOrError = pcall(function(data)
            ...
            if not cursorPlayer then return "No player detected on the cursor, try again." end

            if cursorPlayer == player.id() then return "You can't select yourself for /chid, pick someone else." end
    ...
    starcustomchat.utils.setMessageHandler("/nametag", function(_, _, data)
        local status, resultOrError = pcall(function(data)
            ...
            elseif
                root.assetJson("/player.config:genericScriptContexts").OpenStarbound ~= nil and player.setNametag
            then
...
function xDPC:onSendMessage(data)
    ...
    if data.mode == "xDPC::Prox" then
        ...
        local function sendMessageToPlayers()
            ...
            if position then
                ...
                data.skipRecog = player.getProperty("xDPC::skipRecog") or false
                data.recogGroup = player.getProperty("xDPC::recogGroup") or false

                if root.getConfiguration("xDPC::sendProxChatInLocal") then
                    ...
                    chat.send(xDPCPrefix .. chatTags .. data.content, "Local", false, data)
                else
                ...
                if #globalStrings ~= 0 then
                    ...
                    globalMsg = xDPCPrefix .. chatTags .. globalMsg
                ...
                if #globalOocStrings ~= 0 then
                    ...
                    globalOocMsg = xDPCPrefix .. globalOocMsg
                    -- The third parameter is ignored on StarExtensions, but retains the "..." chat bubble on xStarbound and OpenStarbound.
...
function xDPC:formatIncomingMessage(rawMessage)
    local messageFormatter = function(message)
        ...
        if
            self.serverDefault
            and message.mode == "Broadcast"
            and message.connection == 0
            and message.text:find("connected")
        then
            message.text = ""
        end
        ...
        if
            hasPrefix
            or (
                root.getConfiguration("xDPC::localChatIsProx")
                and (message.mode == "Local" or message.isSccrp or isSccrpMessage)
            )
        then
            message.mode = "Proximity"
            if hasPrefix and not message.processed then message.text = message.text:sub(#xDPCPrefix + 1, -1) end
                    ...
                    if xsb and not message.isSccrp then -- FezzedOne: Already handled in SCCRP with my PR.
                        if copiedMessage or message.targetId then -- FezzedOne: Show the receiver's name for disambiguation on xClient.
                            if world.entityExists(receiverEntityId) then
                                ...
                                if receiverEntityId ~= player.id() then message.mode = "xDPC::ProxSecondary" end
                            end
                        end
                    end
                    do
                            ...
                            ["["] = function()
                                ...
                                elseif fStart ~= nil and fEnd ~= nil then
                                    ...
                                    if tonumber(newCode) or newCode == "-" then -- FezzedOne: This is a comm code.
                                        local newCommCode = newCode == "-" and "-" or tostring(tonumber(newCode) or "0")
        ...
        if message.mode == "xDPC::Prox" then message.isDpc = true end
        ...
        then
            ...
            if charRecInfo == true then charRecInfo = nil end
            local useName = message.fakeName or "^#999;???^reset;"

            if
                (message.alias and message.aliasPrio)
                and (
                    not charRecInfo
                    or (charRecInfo and not charRecInfo.manName and message.aliasPrio < charRecInfo.aliasPrio)
                )
            then --if conditions are met
            ...
            if charRecInfo then useName = charRecInfo.savedName end
        ...
        if xsb then
            message.nickname = message.receiverName
                    and ((message.nickname or "^#999;???^reset;") .. " -> " .. message.receiverName)
                or message.nickname
        end
